...
namespace LIS3DH_Own {
    ...
    namespace Setup {
        ...
        void setFreq(LIS3DHSettings settings) {
            uint8_t curr_val = ReadByte(CTRL_REG1);
            curr_val = settings.Freq_to_Byte(curr_val);
            WriteByte(CTRL_REG1, curr_val);
        }

        void setPowerMode(LIS3DHSettings settings) {
            uint8_t curr_val1 = ReadByte(CTRL_REG1);
            uint8_t curr_val4 = ReadByte(CTRL_REG4);
            uint16_t new_val = settings.Power_Mode_to_Byte(curr_val1, curr_val4);
        ...
        void setMaxAccel(LIS3DHSettings settings) {
            uint8_t curr_val = ReadByte(CTRL_REG4);
            curr_val = settings.Max_Accel_to_Byte(curr_val);
        ...
        void SetupAccelerometer(LIS3DHSettings settings) {
            ResetAccelerometer();
            setFreq(settings);
            setPowerMode(settings);
            setMaxAccel(settings);
        }
    }
    ...
    namespace Data {
        ...
        float getXFloat(LIS3DHSettings settings) {
            float div_factor = settings.Calc_Div_Factor();
        ...
        float getYFloat(LIS3DHSettings settings) {
            float div_factor = settings.Calc_Div_Factor();
        ...
        float getZFloat(LIS3DHSettings settings) {
            float div_factor = settings.Calc_Div_Factor();
